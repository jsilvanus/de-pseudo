...
import { pseudonymize, buildPrompt, parseSessionResponse, validateResults, type ResponseFormat, type PseudonymizedDataset } from './lib/core';
...
export default function App() {
  ...
  useEffect(() => { vault.restore().then(data => { if (data) { setDataset(data); setRestored(true); } }).catch(() => undefined); }, []);

  const pseudonymizedText = useMemo(() => dataset ? JSON.stringify(dataset.rows, null, 2) : '', [dataset]);
  const prompt = useMemo(() => dataset && vault.sessionId ? buildPrompt(dataset.rows, task, format, vault.sessionId) : '', [dataset, task, format]);
  ...
  async function handleResolve() {
    try {
      ...
      setError(''); setResolved(resolveResult(aiResult, dataset.mapping));
  ...
  return <Container maxWidth="md" sx={{ py: 4 }}><Stack spacing={3}>
    ...
    {!dataset && <Paper sx={{ p: 3 }}><Stack spacing={2}><Typography variant="h5">1. Your data</Typography><Typography variant="body2" color="text.secondary">Paste JSON array or tab-separated data. This field is cleared immediately after successful pseudonymization.</Typography><TextField multiline minRows={7} value={input} onChange={e => setInput(e.target.value)} label="Personal data" fullWidth autoComplete="off" /><Button variant="contained" onClick={handlePseudonymize}>Pseudonymize & save locally</Button></Stack></Paper>}
    {dataset && <>
      <Paper sx={{ p: 3 }}><Stack spacing={2}><Typography variant="h5">1. Pseudonymized data</Typography><Typography variant="body2" color="text.secondary">Safe to copy to an external AI: usernames have been removed and replaced with pseudonyms.</Typography><TextField multiline minRows={7} value={pseudonymizedText} InputProps={{ readOnly: true }} fullWidth /><Button variant="outlined" onClick={() => copy(pseudonymizedText, 'data')}>{copied === 'data' ? 'Copied' : 'Copy pseudonymized data'}</Button></Stack></Paper>
      <Paper sx={{ p: 3 }}><Stack spacing={2}><Typography variant="h5">2. AI prompt</Typography><Typography variant="body2" color="text.secondary">The random session ID lets the app recognize the corresponding AI response. It contains no personal information and does not reveal the local mapping.</Typography><TextField label="Task" value={task} onChange={e => setTask(e.target.value)} fullWidth /><FormControl fullWidth><InputLabel>Response format</InputLabel><Select label="Response format" value={format} onChange={e => setFormat(e.target.value as ResponseFormat)}><MenuItem value="lines">Simple lines</MenuItem><MenuItem value="json">JSON</MenuItem></Select></FormControl><TextField multiline minRows={8} value={prompt} InputProps={{ readOnly: true }} label="Copy this prompt to your AI" fullWidth /><Button variant="outlined" onClick={() => copy(prompt, 'prompt')}>{copied === 'prompt' ? 'Copied' : 'Copy prompt'}</Button></Stack></Paper>
      <Paper sx={{ p: 3 }}><Stack spacing={2}><Typography variant="h5">3. Paste AI result</Typography><Typography variant="body2" color="text.secondary">The response must contain the exact session ID from the prompt. Without it, this app will not perform identity resolution.</Typography><TextField multiline minRows={7} value={aiResult} onChange={e => setAiResult(e.target.value)} label={format === 'json' ? 'JSON result' : 'Result'} fullWidth autoComplete="off" /><Button variant="contained" onClick={handleResolve}>Validate & resolve locally</Button></Stack></Paper>
