...
export function ReferenceEditor({ rows, columns, referenceColumns, pseudonymizedColumns, references = [], onChange }: Props) {
  ...
  function handlePointerUp(e: React.PointerEvent) { if (drag) finish(e); }

  return <Paper variant="outlined" sx={{ p: 2 }}>
    <Stack spacing={1}><Typography variant="subtitle2">Draw references in the dataset</Typography><Typography variant="body2" color="text.secondary">Drag the ↗ handle from a Reference cell directly onto a pseudonymized person cell.</Typography>
      <Box ref={setRoot} onPointerMove={move} onPointerUp={handlePointerUp} sx={{ position: 'relative', overflow: 'auto', touchAction: 'none', maxHeight: 520 }}>
